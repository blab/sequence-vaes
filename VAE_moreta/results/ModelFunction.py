...
class DRAUPNIRModelClass(nn.Module):
    ...
    def gp_prior(self,patristic_matrix_sorted):
        ...
        alpha = DraupnirUtils.squeeze_tensor(1,alpha).to(self.device)
        sigma_f = DraupnirUtils.squeeze_tensor(1,sigma_f).to(self.device)
        sigma_n = DraupnirUtils.squeeze_tensor(1,sigma_n).to(self.device)
        lambd = DraupnirUtils.squeeze_tensor(1,lambd).to(self.device)
        ...
        OU_mean = torch.zeros((patristic_matrix.shape[0],)).unsqueeze(0).to(self.device)
        ...
        latent_space = pyro.sample('latent_z', dist.MultivariateNormal(OU_mean, OU_covariance).to_event(1)).to(self.device) #[z_dim=30,n_nodes] #+ noise[None,:,:]
    ...
    def conditional_sampling(self,map_estimates, patristic_matrix):
            ...
            sigma_f = DraupnirUtils.squeeze_tensor(1,map_estimates["sigma_f"]).to(self.device)
            sigma_n = DraupnirUtils.squeeze_tensor(1,map_estimates["sigma_n"]).to(self.device)
            lambd = DraupnirUtils.squeeze_tensor(1,map_estimates["lambd"]).to(self.device)
            ...
            # Highlight:µb
            OU_mean_leaves = torch.zeros((self.n_leaves,)).to(self.device)
